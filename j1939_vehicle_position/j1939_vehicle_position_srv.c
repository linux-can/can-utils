					  ...
					  uint16_t *date, uint32_t *time)
{
	...
	if (date) {
		...
		*date = htole16(time_secs / 86400);
	}

	if (time) {
		/* Calculate the time of day in 0.0001 seconds since midnight */
		*time = htole32((time_secs % 86400) * 10000 + gps_nsec / 100000);
	}
}
...
static void simulate_gps_data(struct j1939_vp_srv_priv *priv)
{
	...
	clock_gettime(CLOCK_REALTIME, &priv->gps_data.fix.time);
			      ...
			      struct j1939_vp2_packet *vp2p)
{
	...
	if (priv->gps_data.set & DOP_SET) {
		...
		vp2p->vdop = (uint8_t)(priv->gps_data.dop.vdop * 10);
		vp2p->pdop = (uint8_t)(priv->gps_data.dop.pdop * 10);
		vp2p->tdop = (uint8_t)(priv->gps_data.dop.tdop * 10);
	} else {
		vp2p->hdop = UINT8_MAX;
		vp2p->vdop = UINT8_MAX;
		vp2p->pdop = UINT8_MAX;
...
{
	struct j1939_vp2_packet *vp2p = (struct j1939_vp2_packet *)data;
			     ...
			     struct j1939_vp1_packet *vp1p)
{
	...
	if (priv->gps_data.set & LATLON_SET) {
		latitude = priv->gps_data.fix.latitude;
		longitude = priv->gps_data.fix.longitude;

		lat_raw = (uint32_t)((latitude + 210.0) * 1e7);
		lon_raw = (uint32_t)((longitude + 210.0) * 1e7);

		vp1p->latitude = htole32(lat_raw);
		vp1p->longitude = htole32(lon_raw);
	} else {
		vp1p->latitude = UINT32_MAX;
		vp1p->longitude = UINT32_MAX;
	}
...
{
	struct j1939_vp1_packet *vp1p = (struct j1939_vp1_packet *)data;
				      ...
				      struct nmea2000_sys_time_packet *stp)
{
	...
	if (priv->sim_mode)
		stp->source = NMEA2000_SYS_TIME_SOURCE_LOCAL_CRYSTAL;
	else
		stp->source = NMEA2000_SYS_TIME_SOURCE_GPS;
	...
	if (priv->gps_data.set & TIME_SET) {
		...
		stp->date = htole16(nmea2000_date);
		stp->time = htole32(nmea2000_time);
	} else {
		stp->date = UINT16_MAX;
		stp->time = UINT32_MAX;
	}
				     ...
				     struct nmea2000_mag_var_packet *mvp)
{
	...
	mvp->sid = priv->sid;
	mvp->variation_source = MAGNETIC_VARIATION_MANUAL;
	mvp->reserved = 0xf;
	mvp->age_of_service = htole32(UINT32_MAX);
	mvp->variation = htole16(UINT16_MAX);
...
{
	struct nmea2000_mag_var_packet *mvp =
		(struct nmea2000_mag_var_packet *)data;
				...
				struct nmea2000_position_rapid_packet *prp)
{
	if (priv->gps_data.set & LATLON_SET) {
		...
		prp->latitude = htole32((int32_t)(latitude * 1e7));
		prp->longitude = htole32((int32_t)(longitude * 1e7));
	} else {
		prp->latitude = INT32_MAX;
		prp->longitude = INT32_MAX;
	}
...
{
	struct nmea2000_position_rapid_packet *prp =
		(struct nmea2000_position_rapid_packet *)data;
...
{
	csr->sid = priv->sid;
	csr->cog_reference = NMEA2000_COG_REFERENCE_ERROR;
	csr->reserved1 = 0x3f;
	...
	if (!(priv->gps_data.set & TRACK_SET))
		...
		/* COG in 1e-4 radians */
		csr->cog = htole16((uint16_t)(priv->gps_data.fix.track * 10000));

	if (!(priv->gps_data.set & SPEED_SET))
		csr->sog = UINT16_MAX;
	else
		/* SOG in 1e-2 m/s */
		csr->sog = htole16((uint16_t)(priv->gps_data.fix.speed * 100));
...
{
	struct nmea2000_cog_sog_rapid_packet *csr =
		(struct nmea2000_cog_sog_rapid_packet *)data;
 ...
 * system type.
 */
static uint8_t gpsd_system_to_nmea2000_gnss_system_type(int system)
{
	switch (system) {
	case NAVSYSTEM_GPS:
 ...
 */
static uint8_t gpsd_mode_to_nmea2000_gnss_method(int mode)
{
	switch (mode) {
	case MODE_NO_FIX:
				...
				struct nmea2000_gnss_position_data_packet *gpdp)
{
	...
	if (priv->gps_data.set & TIME_SET) {
		...
		gpdp->date = htole16(nmea2000_date);
		gpdp->time = htole32(nmea2000_time);
	} else {
		gpdp->date = UINT16_MAX;
		gpdp->time = UINT32_MAX;
	}

	if (priv->gps_data.set & LATLON_SET) {
		gpdp->latitude = htole64(priv->gps_data.fix.latitude * 1e16);
		gpdp->longitude = htole64(priv->gps_data.fix.longitude * 1e16);
	} else {
		gpdp->latitude = INT64_MAX;
		gpdp->longitude = INT64_MAX;
	}

	if (priv->gps_data.set & ALTITUDE_SET) {
		gpdp->altitude = htole64(priv->gps_data.fix.altitude * 1e6);
	} else {
		gpdp->altitude = INT64_MAX;
	}
	 ...
	 */
	gpdp->gnss_type =
		gpsd_system_to_nmea2000_gnss_system_type(NAVSYSTEM_GPS);

	if (priv->sim_mode)
		gpdp->gnss_method = GNSS_METHOD_SIMULATE_MODE;
	else
		gpdp->gnss_method =
			gpsd_mode_to_nmea2000_gnss_method(priv->gps_data.fix.mode);

	gpdp->integrity = 0; /* Default to no integrity checking */

	gpdp->num_svs = priv->gps_data.satellites_visible;

	if (priv->gps_data.set & DOP_SET) {
		gpdp->hdop = htole16((int16_t)(priv->gps_data.dop.hdop * 100));
		gpdp->pdop = htole16((int16_t)(priv->gps_data.dop.pdop * 100));
	} else {
		gpdp->hdop = INT16_MAX;
		gpdp->pdop = INT16_MAX;
	}

	gpdp->geoidal_separation = INT32_MAX;

	gpdp->num_ref_stations = 0;
...
{
	struct nmea2000_gnss_position_data_packet *gpdp =
		(struct nmea2000_gnss_position_data_packet *)data;
